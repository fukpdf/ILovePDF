...
const checks=[
 ...
 ['OCR WorkerPool preserved',/async function _recognizeOcrWithSharedWorker[\s\S]*?WorkerPool\.run\(\s*OCR_WORKER/.test(app)],
 ['render WorkerPool preserved',/async function _renderOcrPageWithSharedWorker[\s\S]*?WorkerPool\.run\(\s*RENDER_WORKER/.test(app)],
