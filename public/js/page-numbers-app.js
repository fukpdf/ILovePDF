// PageNumbersApp v1.0 — Isolated Add Page Numbers Tool (Phase 3 Full Isolation)
// Draws page number labels on every page using pdf-lib.
// Spawns a dedicated pdf-lib-worker.js per job. Terminated after result.
// ADDITIVE ONLY — zero changes to any existing file.
(function (G) {
  'use strict';

  var TAG             = '[PageNumbersApp]';
  ...
  function _runWorker(buffer, opts, jobId) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        ...
        _worker = null;
        reject(new Error('Page numbers worker timed out.'));
      }, WORKER_LIMIT_MS);
      ...
      w.postMessage({ op: 'page-numbers', buffers: [xfer], opts: opts, jobId: String(jobId) }, [xfer]);
    });
  }
  ...
  async function process(files, opts) {
    ...
    var hardPromise = new Promise(function (_, reject) {
      ...
      _hardTimer  = setTimeout(function () {
        ...
        reject(new Error('Page numbers timed out. Please try with a smaller file.'));
      }, HARD_LIMIT_MS);
    });
    ...
    var jobPromise = (async function () {
      ...
      onStep(0, 'done', 20);
      onStep(1, 'active', 25, 'Numbering pages\u2026');
