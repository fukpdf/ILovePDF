// Page Numbers Worker Adapter v1.0 — canonical RuntimeWorkers + adaptive streaming
(function () {
  'use strict';
  if (window.PageNumbersWorkerAdapter) return;
  var WORKER_URL = '/workers/pdf-worker.js';
  var TIMEOUT_MS = 0;
  var STREAM_THRESHOLD = 10 * 1024 * 1024;
  function key(file, opts) {
    opts = opts || {};
    return 'page-numbers:' + String(file && file.name || '') + ':' + String(file && file.size || 0) + ':' +
      String(file && file.lastModified || 0) + ':' + String(opts.startFrom || '1') + ':' + String(opts.position || 'bottom-center');
  }
  async function dispatch(file, opts, onProgress, token) {
    ...
    if (file.size >= STREAM_THRESHOLD && window.RuntimeStreamBridge &&
      ...
      onProgress(5, 'Preparing streaming page numbering…');
      var streamed = await window.RuntimeStreamBridge.pipelineStreamToWorker(
        WORKER_URL, file, { tool: 'page-numbers', options: opts },
    ...
    if (!window.RuntimeWorkers || typeof window.RuntimeWorkers.dispatch !== 'function') {
      throw new Error('RuntimeWorkers is unavailable — canonical Page Numbers worker runtime cannot dispatch');
    }
    onProgress(10, 'Reading PDF…');
    var buffer = await file.arrayBuffer();
    if (!buffer || !buffer.byteLength) throw new Error('Page Numbers input is empty');
    if (token && token.cancelled) throw new Error('cancelled-after-read');
    var result = await window.RuntimeWorkers.dispatch(
      WORKER_URL,
      { tool: 'page-numbers', buffers: [buffer], options: opts },
      [buffer],
      { priority: 'normal', label: 'page-numbers-worker', dedupeKey: key(file, opts), timeoutMs: TIMEOUT_MS, token: token }
    );
    if (!result || !result.buffer || !result.buffer.byteLength) throw new Error('Worker produced empty output');
    onProgress(100, 'Done!');
    return { buffer: result.buffer };
  }
  window.PageNumbersWorkerAdapter = { dispatch: dispatch, WORKER_URL: WORKER_URL, TIMEOUT_MS: TIMEOUT_MS, STREAM_THRESHOLD: STREAM_THRESHOLD };
}());
