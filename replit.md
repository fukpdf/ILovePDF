...
- **Quality Score Enforcement (v5.2)**: `analyzeResultQuality()` uses hybrid scoring — 65% output-size ratio + 35% structural metadata (chars, paras, rows, pages, ocrUsed). Score < 0.40 on binary-output tools (`_BINARY_TOOLS` Set) throws `low_quality_output` and blocks download. Score 0.40–0.69 logs a warning only.
