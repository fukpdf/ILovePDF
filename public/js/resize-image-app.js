// ResizeImageApp v1.0 — Isolated Resize Image Tool (Phase 3 Full Isolation)
...
(function (G) {
  ...
  async function process(files, opts) {
    if (_inFlight) throw new Error('Resize image already in progress');
    if (!files || !files[0]) throw new Error('No file provided');
    _inFlight = true;
    var jobId = ++_jobId;
    var file  = files[0];
    var mime  = _getMime(file);
    _log('start', { job: jobId, file: file.name, preset: opts && opts.preset });
