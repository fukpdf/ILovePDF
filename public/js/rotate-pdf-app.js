// RotatePdfApp v1.0 — Isolated Rotate PDF Tool (Phase 3 Full Isolation)
...
(function (G) {
  ...
  function _runWorker(buffer, opts, jobId) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        ...
        _worker = null;
        reject(new Error('Rotate worker timed out.'));
      }, WORKER_LIMIT_MS);
      ...
      w.postMessage({ op: 'rotate', buffers: [xfer], opts: opts, jobId: String(jobId) }, [xfer]);
    });
  }
  ...
  async function process(files, opts) {
    if (_inFlight) throw new Error('Rotate already in progress');
    if (!files || !files[0]) throw new Error('No file provided');
    _inFlight = true;
    var jobId = ++_jobId;
    var file  = files[0];
    _log('start', { job: jobId, file: file.name, opts: opts });
    ...
    var hardPromise = new Promise(function (_, reject) {
      ...
      _hardTimer  = setTimeout(function () {
        ...
        reject(new Error('Rotate timed out. Please try with a smaller file.'));
      }, HARD_LIMIT_MS);
    });
    ...
    var jobPromise = (async function () {
      ...
      onStep(0, 'done', 20);
      onStep(1, 'active', 25, 'Rotating pages\u2026');
