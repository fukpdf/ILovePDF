// UnlockPdfApp v1.0 — Isolated Unlock PDF Tool (Phase 3 Full Isolation)
// Re-saves with ignoreEncryption:true to strip simple password locks.
// Spawns a dedicated pdf-lib-worker.js per job. Terminated after result.
// ADDITIVE ONLY — zero changes to any existing file.
(function (G) {
  'use strict';

  var TAG             = '[UnlockPdfApp]';
  var TOOL_ID         = 'unlock';
  ...
  function _runWorker(buffer, opts, jobId) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        ...
        _worker = null;
        reject(new Error('Unlock worker timed out.'));
      }, WORKER_LIMIT_MS);
      ...
      w.postMessage({ op: 'unlock', buffers: [xfer], opts: opts, jobId: String(jobId) }, [xfer]);
    });
  }
  ...
  async function process(files, opts) {
    ...
    var hardPromise = new Promise(function (_, reject) {
      ...
      _hardTimer  = setTimeout(function () {
        ...
        reject(new Error('Unlock timed out. Please try with a smaller file.'));
      }, HARD_LIMIT_MS);
    });
    ...
    var jobPromise = (async function () {
      ...
      onStep(0, 'done', 20);
      onStep(1, 'active', 25, 'Removing lock\u2026');
