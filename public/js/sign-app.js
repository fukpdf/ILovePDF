// SignApp v1.0 — Isolated Sign PDF Tool (Phase 3 Full Isolation)
// Upgrades sign-runtime.js: replaces shared WorkerPool with dedicated worker.
// Draws styled signature text on the target page using pdf-lib.
// Spawns a dedicated pdf-lib-worker.js per job. Terminated after result.
// PRIVACY: signature text is never logged in telemetry.
...
(function (G) {
  ...
  function _runWorker(buffer, opts, jobId) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        ...
        _worker = null;
        reject(new Error('Sign worker timed out.'));
      }, WORKER_LIMIT_MS);
      ...
      // PRIVACY: signatureText passed to worker but never logged here
      w.postMessage({ op: 'sign', buffers: [xfer], opts: opts, jobId: String(jobId) }, [xfer]);
    });
  }
  ...
  async function process(files, opts) {
    if (_inFlight) throw new Error('Sign already in progress');
    if (!files || !files[0]) throw new Error('No file provided');
    _inFlight = true;
    var jobId = ++_jobId;
    var file  = files[0];
    // PRIVACY: do NOT log signatureText
    _log('start', { job: jobId, file: file.name, page: opts && opts.page });
    ...
    var hardPromise = new Promise(function (_, reject) {
      ...
      _hardTimer  = setTimeout(function () {
        ...
        reject(new Error('Sign timed out. Please try with a smaller file.'));
      }, HARD_LIMIT_MS);
    });
    ...
    var jobPromise = (async function () {
      ...
      onStep(0, 'done', 20);
      onStep(1, 'active', 25, 'Adding signature\u2026');
