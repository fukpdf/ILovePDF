// WatermarkPdfApp v1.0 — Isolated Watermark PDF Tool (Phase 3 Full Isolation)
// Draws configurable text watermark on every page using pdf-lib.
// Spawns a dedicated pdf-lib-worker.js per job. Terminated after result.
// ADDITIVE ONLY — zero changes to any existing file.
(function (G) {
  'use strict';

  var TAG             = '[WatermarkPdfApp]';
  ...
  function _runWorker(buffer, opts, jobId) {
    return new Promise(function (resolve, reject) {
      ...
      w.postMessage({ op: 'watermark', buffers: [xfer], opts: opts, jobId: String(jobId) }, [xfer]);
    });
  }
  ...
  async function process(files, opts) {
    if (_inFlight) throw new Error('Watermark already in progress');
    ...
    var hardPromise = new Promise(function (_, reject) {
      ...
      _hardTimer  = setTimeout(function () {
        ...
        reject(new Error('Watermark timed out. Please try with a smaller file.'));
      }, HARD_LIMIT_MS);
    });
    ...
    var jobPromise = (async function () {
      ...
      onStep(0, 'done', 20);
      onStep(1, 'active', 25, 'Applying watermark\u2026');
