...
if (browserRuntime.indexOf('active.set(toolId, { key: key, token: runToken })') === -1) fail('BrowserToolRuntime does not bind each active tool execution to its cancellation token.');
...
if (browserRuntime.indexOf('finally {') === -1 || browserRuntime.indexOf('active.delete(toolId);') === -1) fail('BrowserToolRuntime does not remove active tool state after completion or cancellation.');
