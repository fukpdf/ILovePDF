// ProtectPdfApp v1.0 — Isolated Protect PDF Tool (Phase 3 Full Isolation)
// Adds visual password-protection overlay per page using pdf-lib.
// Spawns a dedicated pdf-lib-worker.js per job. Terminated after result.
// ADDITIVE ONLY — zero changes to any existing file.
(function (G) {
  'use strict';

  var TAG             = '[ProtectPdfApp]';
  var TOOL_ID         = 'protect';
  ...
  function _runWorker(buffer, opts, jobId) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        ...
        _worker = null;
        reject(new Error('Protect worker timed out.'));
      }, WORKER_LIMIT_MS);
      ...
      w.postMessage({ op: 'protect', buffers: [xfer], opts: opts, jobId: String(jobId) }, [xfer]);
    });
  }
  ...
  async function process(files, opts) {
    if (_inFlight) throw new Error('Protect already in progress');
    ...
    var hardPromise = new Promise(function (_, reject) {
      ...
      _hardTimer  = setTimeout(function () {
        ...
        reject(new Error('Protect timed out. Please try with a smaller file.'));
      }, HARD_LIMIT_MS);
    });
    ...
    var jobPromise = (async function () {
      ...
      onStep(0, 'done', 20);
      onStep(1, 'active', 25, 'Applying protection\u2026');
