// EditPdfApp v1.0 — Isolated Edit PDF Tool (Phase 3 Full Isolation)
// Upgrades edit-runtime.js: replaces shared WorkerPool with dedicated worker.
// Places text at percentage-based coordinates on target page(s) using pdf-lib.
...
(function (G) {
  ...
  function _runWorker(buffer, opts, jobId) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        ...
        _worker = null;
        reject(new Error('Edit worker timed out.'));
      }, WORKER_LIMIT_MS);
      ...
      w.postMessage({ op: 'edit', buffers: [xfer], opts: opts, jobId: String(jobId) }, [xfer]);
    });
  }
  ...
  async function process(files, opts) {
    ...
    var jobId = ++_jobId;
    var file  = files[0];
    _log('start', { job: jobId, file: file.name, textLen: ((opts && opts.text) || '').length, page: opts && opts.page });
    ...
    var hardPromise = new Promise(function (_, reject) {
      ...
      _hardTimer  = setTimeout(function () {
        ...
        reject(new Error('Edit timed out. Please try with a smaller file.'));
      }, HARD_LIMIT_MS);
    });
    ...
    var jobPromise = (async function () {
      ...
      onStep(0, 'done', 20);
      onStep(1, 'active', 25, 'Placing text\u2026');
