// RedactPdfApp v1.0 — Isolated Redact PDF Tool (Phase 3 Full Isolation)
// Upgrades redact-runtime.js: replaces shared WorkerPool with dedicated worker.
// Draws opaque black rectangles at percentage-based coordinates using pdf-lib.
...
(function (G) {
  ...
  function _runWorker(buffer, opts, jobId) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        ...
        _worker = null;
        reject(new Error('Redact worker timed out.'));
      }, WORKER_LIMIT_MS);
      ...
      w.postMessage({ op: 'redact', buffers: [xfer], opts: opts, jobId: String(jobId) }, [xfer]);
    });
  }
  ...
  async function process(files, opts) {
    ...
    var jobId = ++_jobId;
    var file  = files[0];
    _log('start', { job: jobId, file: file.name, pages: opts && opts.pages });
    ...
    var hardPromise = new Promise(function (_, reject) {
      ...
      _hardTimer  = setTimeout(function () {
        ...
        reject(new Error('Redact timed out. Please try with a smaller file.'));
      }, HARD_LIMIT_MS);
    });
