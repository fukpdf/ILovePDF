// Split Worker Adapter v1.0 — canonical RuntimeWorkers bridge
(function () {
  'use strict';
  ...
  function dedupeKey(file, opts) {
    opts = opts || {};
    return 'split:' + String(file && file.name || '') + ':' +
  ...
  async function dispatch(file, opts, onProgress, token) {
    ...
    if (file.size >= STREAM_THRESHOLD &&
      ...
      var streamed = await window.RuntimeStreamBridge.pipelineStreamToWorker(
        WORKER_URL, file,
        { tool: 'split', options: opts },
        { token: token, onProgress: onProgress, streamThreshold: STREAM_THRESHOLD }
      );
      if (streamed) {
        if (!streamed.buffer || !streamed.buffer.byteLength) {
