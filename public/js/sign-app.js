// SignApp v2.0 — shared browser-worker lifecycle adapter.
// Signature text is passed only as processing input and is never logged.
(function (G) {
  'use strict';
  ...
  async function process(files, opts) {
    ...
    try {
      ...
      // Never log signatureText or any other user-entered signature content.
      _log('start', { job: jobId, file: files[0].name, page: options.page });
      var result = await G.BrowserTools.process(TOOL_ID, files, options);
      if (!result || !result.blob || result.blob.size < 500) {
        throw new Error('Signing produced an invalid PDF.');
      }
      if (!result.filename) result.filename = _filename(files[0].name);
      _log('done', { job: jobId, size: result.blob.size });
      return result;
    } finally {
