// PageNumbersApp v2.0 — shared browser-worker lifecycle adapter.
(function (G) {
  'use strict';
  var TAG='[PageNumbersApp]', TOOL_ID='page-numbers', _inFlight=false, _jobId=0, _cancelToken=null;
  ...
  function _filename(original){
    var base=String(original||'document').replace(/\.[^.]+$/,'');
    return (base.toLowerCase().startsWith('ilovepdf')?base:'ilovepdf-'+base)+'.pdf';
  }
  async function process(files,opts){
    if(_inFlight) throw new Error('Page numbers already in progress');
    ...
    try{
      ...
      _log('start',{job:jobId,file:files[0].name,pageCount:options.startFrom});
      var result=await G.BrowserTools.process(TOOL_ID,files,options);
      if(!result||!result.blob||result.blob.size<500) throw new Error('Page numbering produced an invalid PDF.');
  ...
  function mount(){_log('mounted');} function unmount(){_cancel('unmount');}
  function reset(){_cancel('reset');} function recover(){_cancel('recover');} function destroy(){_cancel('destroy');}
