...
- **Quality Score Enforcement (v5.1)**: `analyzeResultQuality()` scores output size relative to per-tool minimums. Score < 0.40 on binary-output tools (`_BINARY_TOOLS` Set: pdf-to-word, pdf-to-excel, pdf-to-powerpoint, background-remover, repair, compress) throws `low_quality_output` and blocks download. Score 0.40–0.69 logs a warning only.
