// CropImageApp v1.0 — Isolated Crop Image Tool (Phase 3 Full Isolation)
// Uses OffscreenCanvas + createImageBitmap inside dedicated image-tools-worker.js.
// Spawns a fresh worker per job, terminated immediately after result.
// No canvas leak possible — processing entirely off main thread.
// ADDITIVE ONLY — zero changes to any existing file.
(function (G) {
  'use strict';

  var TAG              = '[CropImageApp]';
  var TOOL_ID          = 'crop-image';
  ...
  function _runWorker(buffer, mime, opts, jobId) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        ...
        _worker = null;
        reject(new Error('Crop image worker timed out.'));
      }, WORKER_LIMIT_MS);
      ...
      w.postMessage({ op: 'crop-image', buffer: xfer, mime: mime, opts: opts, jobId: String(jobId) }, [xfer]);
    });
  }

  function _filename(orig, ext) {
    var base = (orig || 'image').replace(/\.[^.]+$/, '');
    return (base.toLowerCase().startsWith('ilovepdf') ? base : 'ilovepdf-' + base) + '.' + ext;
  }
  ...
  async function process(files, opts) {
    if (_inFlight) throw new Error('Crop image already in progress');
    if (!files || !files[0]) throw new Error('No file provided');
    _inFlight = true;
    var jobId = ++_jobId;
    var file  = files[0];
    var mime  = _getMime(file);
    _log('start', { job: jobId, file: file.name, mime: mime });
    ...
    var hardPromise = new Promise(function (_, reject) {
      ...
      _hardTimer  = setTimeout(function () {
        ...
        reject(new Error('Crop image timed out.'));
      }, HARD_LIMIT_MS);
    });
    ...
    var jobPromise = (async function () {
      ...
      return { blob: blob, filename: _filename(file.name, ext), ext: ext, mime: result.mime || mime };
    })();
