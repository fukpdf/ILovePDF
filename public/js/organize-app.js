// OrganizeApp v1.0 — Isolated Organize PDF Tool (Phase 3 Full Isolation)
// Upgrades organize-runtime.js: replaces shared WorkerPool with dedicated worker.
// Spawns a dedicated pdf-lib-worker.js per job. Terminated after result.
// Takes priority over organize-runtime.js via ToolAppManager interceptor chain.
// ADDITIVE ONLY — zero changes to any existing file.
(function (G) {
  'use strict';

  var TAG             = '[OrganizeApp]';
  var TOOL_ID         = 'organize';
  ...
  function _runWorker(buffer, opts, jobId) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = setTimeout(function () {
        ...
        _worker = null;
        reject(new Error('Organize worker timed out.'));
      }, WORKER_LIMIT_MS);
  ...
  async function process(files, opts) {
    if (_inFlight) throw new Error('Organize already in progress');
    if (!files || !files[0]) throw new Error('No file provided');
    _inFlight = true;
    var jobId = ++_jobId;
    var file  = files[0];
    _log('start', { job: jobId, file: file.name, pageOrder: opts && opts.pageOrder });
    ...
    var jobPromise = (async function () {
      ...
      onStep(0, 'done', 20);
      onStep(1, 'active', 25, 'Reordering pages\u2026');
